...
INPUT_SIZE = 64
BATCH_SIZE = 1
data_dir = './candidates_v2_0.75mm_32_3d/train2_new_tianci_subset[0]/True/1361.pkl.gz'
import sys
...
def create_record():
    ...
    writer = tf.python_io.TFRecordWriter("/tmp/test.bin")
...
def cnn_network(x):
    weights = {'W_conv1':tf.Variable(tf.random_normal([3,3,3,1,32])),
               ...
               'W_fc':tf.Variable(tf.random_normal([16777216,420])),
               'out':tf.Variable(tf.random_normal([420,2]))}
    biases = {'b_conv1':tf.Variable(tf.random_normal([32])),
              'b_conv2':tf.Variable(tf.random_normal([64])),
              'b_fc':tf.Variable(tf.random_normal([420])),
              'out':tf.Variable(tf.random_normal([2]))}
    x = tf.reshape(x, shape=[-1, 64,64,64, 1])
    ...
    fc = tf.reshape(conv2, [-1, dim])
    fc = tf.contrib.layers.batch_norm(tf.matmul(fc, weights['W_fc'])+biases['b_fc'], center=True,is_training = True, scale =True)

    fc = tf.nn.relu(fc)
    fc = tf.nn.dropout(fc, 0.8)

    out_put = tf.matmul(fc, weights['out'])+biases['out']
...
if __name__ == '__main__':
    ...
    # quit()
    img, label = read_and_decode_single("/tmp/large_train.bin")
    ...
    y_pred = new_3dnet.inference(img_batch)
    # y_pred = cnn_network(img_batch)
    ...
    # loss = new_3dnet.loss(y_pred, label_batch)
    loss_mean = tf.reduce_mean(loss)

    # train_op = tf.train.AdamOptimizer().minimize(loss)
    train_op = new_3dnet.train(loss, global_step)
    #初始化所有的op
    ...
    with tf.Session() as sess:
        ...
        for i in range(0,3):
            ...
            _,loss_val = sess.run([train_op,img_batch])
            #l = to_categorical(l, 12)
            # print(val.shape, l)
            # print y_pred
            cifar10_input_test.assertAllEqual(expected, loss_val)
            print("Worked!!!")